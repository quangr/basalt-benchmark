...
def embed_trajectories(data_dir, in_model, in_weights):
    ...
    agent.policy.pi_head.reset_parameters()
    for param in policy.pi_head.parameters():
        param.requires_grad = True

    # Define the optimizer (e.g., Adam optimizer)
    optimizer = torch.optim.Adam(policy.pi_head.parameters(), lr=0.001)
    ...
    for epoch in range(1000):
        ...
        for batch_episode_id, batches in data_loader:
            ...
            log_loss = -policy.pi_head.logprob(
                {
        ...
        if (epoch + 1) % 100 == 0:
            save_path = f"pipeline_test_data/pi_head/epoch_{epoch + 1}.pt"
            torch.save(policy.pi_head, save_path)
