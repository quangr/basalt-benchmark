...
def add_rollout_specific_args(parser):
    parser.add_argument(
        "--output_dir",
        type=str,
        default="pipeline_test_data/rollouts/cls/MineRLBasaltMakeWaterfall-v0",
    ...
    parser.add_argument(
        "--agent_file",
        type=str,
        default="pipeline_test_data/cls/epoch_100.pt",
...
def main(args):
    ...
    v_head = torch.load(args.agent_file)
    ...
    for seed in tqdm.tqdm(environment_seeds, desc="Seeds", leave=False):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                v = v_head(vpt_embedding)
                w=10
                input_pd={key:pd[key]+w*v[key] for key in v.keys()}
                ac = vpt_agent.policy.pi_head.sample(input_pd, deterministic=False)
