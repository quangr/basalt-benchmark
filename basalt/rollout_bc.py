...
def add_rollout_specific_args(parser):
    ...
    parser.add_argument(
        "--agent_file",
        type=str,
        default="pipeline_test_data/pi_head/epoch_100.pt",
...
def main(args):
    ...
    pi_head = torch.load(args.agent_file)
    ...
    for seed in tqdm.tqdm(environment_seeds, desc="Seeds", leave=False):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                # old_pd = vpt_agent.policy.pi_head(vpt_embedding)
                pd = pi_head(vpt_embedding)
                input_pd=pd
                # w=5
                # input_pd={key:(1+w)*pd[key]-w*old_pd[key] for key in old_pd.keys()}
                ac = pi_head.sample(input_pd, deterministic=False)
