...
from basalt.utils.toy_data_loader import DataLoader
from basalt.vpt_lib.tree_util import tree_map
...
def embed_trajectories(data_dir, in_model, in_weights, out_dir, check_accuracies=False):
    ...
    episode_progress_bar = tqdm.tqdm(
        desc="Episode", total=len(data_loader.demonstration_tuples)
    )
    ...
    for batch_i, (batch_images, batch_actions, batch_episode_id) in enumerate(
        ...
        for image, action, episode_id in zip(
            ...
            if image is None and action is None:
                ...
                if episode_id in episode_embeddings:
                    (
                        ...
                        esc_actions,
                        is_null_action,
                    ) = episode_embeddings.pop(episode_id)
            ...
            with th.no_grad():
                if check_accuracies:
                    predicted_act, new_agent_state, _ = policy.act(
                        agent_obs, dummy_first, agent_state, stochastic=False
                    )
                    ...
                    accuracy_metrics["button_corrects"] += int(
                        button_prediction == true_button
                    )
                    accuracy_metrics["camera_corrects"] += int(
                        camera_prediction == true_camera
                    )
                    ...
                    episode_embeddings[episode_id][1].append(
                        embedding.detach().cpu().numpy().astype(np.float32)[0, 0]
                    )
                    episode_embeddings[episode_id][2].append(
                        int(agent_action["buttons"].item())
                    )
                    episode_embeddings[episode_id][3].append(
                        int(agent_action["camera"].item())
                    )
                    episode_embeddings[episode_id][4].append(int(action["ESC"]))
                    episode_embeddings[episode_id][5].append(
                        int(action["is_null_action"])
                    )
        ...
        if check_accuracies and (batch_i % REPORT_RATE) == 0:
            ...
            tqdm.tqdm.write(
                f"\tButton accuracy: {accuracy_metrics['button_corrects'] / accuracy_metrics['total']}"
            )
            tqdm.tqdm.write(
                f"\tCamera accuracy: {accuracy_metrics['camera_corrects'] / accuracy_metrics['total']}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out-dir", default="pipeline_test_data/embeddings/foundation-model-3x.weights", type=str, help="Path where embeddings are stored"
    )
    parser.add_argument(
        "--check-accuracies",
        action="store_true",
        help="Instead of saving embeddings, save accuracies",
    )
